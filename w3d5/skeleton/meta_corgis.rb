...
class MetaCorgiSnacks
  ...
  #   cap_label = label[0].upcase + label[1..-1]
  #   info = "get_#{method_name}_info"
  #   score = "get_#{method_name}_tastiness"
  #   snack_data = "#{cap_label}: #{@snack_box.send info, @box_id}: #{@snack_box.send score, @box_id}"
  #
  #   if (@snack_box.send score, @box_id).to_i > 30
  #     return "* " + snack_data
  #   end
  #
  #   snack_data
  ...
  def self.define_snack(name)
    define_method(name) {
      ...
      score = @snack_box.send("get_#{name}_tastiness", @box_id)
      display_name = name.capitalize
      result = "#{display_name}: #{info}: #{score}"
      score > 30 ? "* #{result}" : result
    }
  end
end
